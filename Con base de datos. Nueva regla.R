...
# Se agregó otra regla para que solo fueran elegibles niños menores de 9 años
# En la fila 34,56 y 101


setwd("C:/Users/consu/OneDrive/Documents/Curso Modelación basada en agentes")
library(ABM)
...
season_vec <- c(              # Estacionalidad y política de puntualidad pública. Efectos estacionales por mes (enero..diciembre)
  0.00,  0.00, -0.03, -0.03,  # ene, feb, mar, abr
  0.00,  0.02,  0.02,  0.00,  # may, jun, jul, ago
  -0.02, -0.02,  -0.05,  -0.05  # sep, oct, nov, dic
)
...
for (i in 1:N) {              # Inicia un bucle que va del agente 1 al N. i es el índice del agente (ID interno en sim)
  sim$setState(               # Asigna el estado y atributos del agente i.
    i,                        # En resumen, toda esta parte agrega a cada fila un resultado de la simulación dadas estas condiciones
    list(
      hogares$state0[i],
...
tick_handler <- function(time, sim, agent) {      # Esta función se ejecutará una vez por cada unidad de tiempo (tick) en la simulación. El modelo ABM la volverá a llamar automáticamente en cada paso.
  ...
  season_eff <- season_vec[(t %% 12) + 1]              # Estacionalidad según el mes (Aquí se está indicando que se repite cada 12 periodos)
  ...
  for (i in 1:N) {                                # recorre los N hogares.
    ...
    # Es una forma de penalizar. Ejemplo microeempresas reciben menos crédito. En este caso es la puntualidad percibida por el hogar (usa el valor del mes y ajusta por zona - 5%)
    ...
    tau <- 500  # temperatura/margen de error/tolerancia al cambio:
    # se ajusta en pesos por ejemplo. Controla cuánta aleatoriedad permites:
    
    
    u_center <- utilities - max(utilities) # estabilidad numérica: restar la mayor utilidad antes de experiencia. 
